...
const generatedVerifier = path.join(generatedDir, "NoirOperationGeneratedVerifier.sol");
...
function renameGeneratedVerifier(source) {
  const patterns = [
    /contract\s+HonkVerifier\s+is\s+/,
    /contract\s+UltraVerifier\s+is\s+/,
    /contract\s+[A-Za-z0-9_]*Verifier\s+is\s+/,
  ];
  for (const pattern of patterns) {
    if (pattern.test(source)) {
      return source.replace(pattern, "contract NoirOperationGeneratedVerifier is ");
    }
  }
  const plainVerifier = /contract\s+[A-Za-z0-9_]*Verifier\s*\{/;
  if (plainVerifier.test(source)) {
    return source.replace(plainVerifier, "contract NoirOperationGeneratedVerifier {");
  }
...
console.log("\nNoir operation authority artifacts generated:");
